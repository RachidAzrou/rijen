  ...
  return (
    <div className="min-h-screen w-full" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="container mx-auto px-4 py-6 md:py-8 space-y-6 md:space-y-8">
            ...
            {language === 'nl' ? (
              <>
                ...
                <div className="text-center">
                  <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-[#963E56] mb-1">
                    {t.pageTitle}
                  </h1>
                  <p className="text-sm md:text-base text-[#963E56]/70">
                    Live status overzicht
                  </p>
                </div>
              </>
            ) : (
            ...
            >
              <CardHeader className="p-4 md:p-6 pb-2 md:pb-4 flex flex-row items-center justify-between space-y-0">
                <CardTitle className={`flex items-center gap-3 text-base md:text-lg font-semibold text-[#963E56] ${language === 'ar' ? 'flex-row-reverse' : ''}`}>
                  {language === 'nl' ? (
                    <>
                      <FaPray className="h-5 w-5 md:h-6 md:w-6" />
                ...
                <div className={`
                  ...
                  ${statusMap[id as RoomId] === 'green' ? 'bg-[#6BB85C] shadow-lg shadow-[#6BB85C]/50' :
                    ...
                    'bg-gray-300'}
                `}>
                  {statusMap[id as RoomId] === 'green' && <Check className="w-6 h-6 md:w-7 md:h-7 text-white" />}
                  {statusMap[id as RoomId] === 'red' && <X className="w-6 h-6 md:w-7 md:h-7 text-white" />}
                </div>
              </CardHeader>
              <CardContent className="p-4 md:p-6 pt-2">
                <div className="mt-4 h-2 md:h-3 w-full bg-gray-100 rounded-full overflow-hidden">
                  <div
                ...
                <div className="mt-4 text-center">
                  {statusMap[id as RoomId] !== 'grey' && (
                    <span className={`
                      inline-block px-4 py-1.5 rounded-full text-sm md:text-base font-medium
        ...
        <div className="text-center text-sm md:text-base text-[#963E56]/70 mt-8">
    ...
    <CardContent className="p-6 md:p-8">
      {language === 'nl' ? (
        <blockquote className="space-y-4 text-center">
          ...
          <footer className="text-sm md:text-base text-[#963E56]/80 mt-4">
            — {t.hadithSource}
          </footer>
        </blockquote>
      ) : (
        <div className="space-y-6 text-center" dir="rtl">
          <p className="text-xl md:text-2xl lg:text-3xl text-[#963E56] leading-relaxed font-medium">
            {t.hadithTitle}
          </p>
          <p className="text-xl md:text-2xl lg:text-3xl text-[#963E56] leading-relaxed font-medium">
            {t.hadithText}
          </p>
          <p className="text-sm md:text-base text-[#963E56]/80 mt-4">
            {t.hadithSource}
          </p>
        </div>
      )}
