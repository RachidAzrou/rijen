...
const HadiethCard = () => (
  <Card className="bg-gradient-to-br from-[#FF3B30]/5 to-transparent border border-[#FF3B30]/10">
    <CardContent className="p-4">
      <blockquote className="space-y-2">
        <p className="text-base text-[#FF3B30] leading-relaxed font-medium italic">
          "Houd de rijen recht, want het recht houden van de rijen is deel van het perfect verrichten van het gebed."
        </p>
        <footer className="text-xs text-[#FF3B30]/80">
...
export function SufufPage() {
  ...
  return (
    <div
      className="min-h-screen px-4 py-6 space-y-6"
      style={{
        backgroundImage: `linear-gradient(135deg, rgba(245, 247, 250, 0.7), rgba(195, 207, 226, 0.7)), url('/static/123.jpg')`,
    ...
    >
      <div className="container mx-auto">
        ...
        <div className="flex items-center gap-4">
          <PiUsersThree className="h-8 w-8 text-[#FF3B30]" />
          <h1 className="text-2xl md:text-3xl font-bold text-[#FF3B30]">
            Sufuf (Gebedsrijen)
          </h1>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-[#FF3B30] flex items-center gap-2">
            ...
            {Object.values(rooms).map((room) => (
              ...
              >
                <CardHeader className="p-6 pb-4 flex flex-row items-center justify-between space-y-0">
                  <CardTitle className="flex items-center gap-3 text-lg font-semibold text-[#FF3B30]">
                  ...
                  <div className={`
                    ...
                    ${room.status === 'green' ? 'bg-[#6BB85C] animate-pulse shadow-lg shadow-[#6BB85C]/50' :
                      room.status === 'red' ? 'bg-[#FF3B30] animate-pulse shadow-lg shadow-[#FF3B30]/50' :
                      'bg-gray-300'}
                  `}>
                  ...
                  <div className="mt-4 h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all duration-500 ${
                        ...
                        room.status === 'red' ? 'w-full bg-[#FF3B30]' :
                        'w-0'
                      }`}
                    />
                  </div>
        ...
        <div className="space-y-4">
          <Button
            variant="ghost"
            className="w-full flex items-center justify-between p-2 text-xl font-semibold text-[#FF3B30] hover:bg-[#FF3B30]/5"
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-[#FF3B30]">{room.title}</span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm border border-gray-100">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-[#FF3B30]">OK</span>
                      </div>
                    ...
                    <div className="mt-4 flex items-center justify-between p-3 bg-white rounded-lg shadow-sm border border-gray-100">
                      <div className="flex items-center gap-3">
                        <X className={`w-5 h-5 ${room.status === 'red' ? 'text-[#FF3B30]' : 'text-gray-300'}`} />
                        <span className="font-medium text-[#FF3B30]">NOK</span>
                      </div>
                      <label className="relative inline-block w-12 h-6">
                        <input
                          type="checkbox"
                        ...
                        <span className={`
                          absolute cursor-pointer inset-0 rounded-full transition-all duration-300
                          ${room.status === 'red' ? 'bg-[#FF3B30]' : 'bg-gray-200'}
                        `} />
                        <span className={`
                          absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform duration-300
